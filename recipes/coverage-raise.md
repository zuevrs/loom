...
You are running unattended. Contract: `docs/unattended.md` — dedicated branch, commits allowed, `loom-verify` before the report, never merge. Change tier: this run modifies test code (and only test code).
...
1. **Pre-flight baseline:** run the test suite. Already red → stop, write a report recording the failures; don't build on a red base.
...
- A report whose summary describes the behavior now protected in the selected project language — tests only, suite green, coverage delta for the module in the description, verify digest included; do not use the recipe name as the title.
- Any discovered bugs as `needs-triage` stubs in the same report.
...
- Suite must be green before the report.
