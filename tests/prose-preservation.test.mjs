...
test("OMP and Orca keep runtime native while Loom keeps durable meaning",()=>{
  ...
  assert.match(omp,/skills\/prose-only by default/is);assert.match(omp,/optional `runtime-guard` feature.*stateless.*cannot mutate Story\/Ticket disposition/is);
...
test("session draft stages confirmed boundary events without becoming durable memory",()=>{
  ...
  assert.match(dispatcher,/SESSION\.md[\s\S]*create no empty draft.*Lazy-create.*qualifying decision/is);
