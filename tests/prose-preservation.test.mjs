...
test("native automation safety is cross-host and bounded",()=>{const docs=read("docs/unattended.md"),omp=read("skills/loom/OMP.md"),implement=read("skills/loom-implement/SKILL.md");for(const anchor of ["every host","single-pass, finite, bounded attempt","report-only","structured result fields","silent death is forbidden","same unchanged error twice","no third identical attempt","zero findings","no project write","no commit, push","independent checker"])assert.match(docs,new RegExp(anchor.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),"i"),`native automation lost ${anchor}`);assert.match(omp,/unchanged error twice|no third identical attempt|bounded/i);assert.match(omp,/apply to native automation and worker attempts, not to the `session_stop` diagnostic callback/);assert.match(omp,/never prevents a stop|cannot prevent a stop/);assert.match(omp,/never returns `continue: true`|never forces another model turn/);assert.match(omp,/never fires for subagents/);assert.match(omp,/`tool_call` event returns `\{ block, reason \}`/);assert.doesNotMatch(omp,/one corrective lap|bounded exit path/);assert.match(implement,/two overlapping REJECTs|two strikes rule/i)});
...
test("OMP and Orca keep runtime native while Loom keeps durable meaning",()=>{
  ...
  assert.match(omp,/callbacks.*diagnostic evidence only.*cannot mutate Story or Ticket disposition/is);
