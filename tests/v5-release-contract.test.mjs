...
const pkg=JSON.parse(read("package.json")); equal(pkg.version,"6.0.0","current candidate must carry the release version");
const packed=JSON.parse(execFileSync("npm",["pack","--dry-run","--json"],{cwd:root,encoding:"utf8"}))[0].files.map(x=>x.path);
for(const wanted of ["docs/migration-v5.md","docs/migration-v6.md","docs/evidence/V5-MIGRATION-PILOT.md","docs/evidence/V6-ATTENDED-PILOT.md","docs/evidence/v5-release-ledger.md","hooks/lane-evidence.cjs","hooks/v6-contracts.cjs","hooks/v6-migration.cjs","scripts/v5-migration-pilot","scripts/v6-attended-pilot","tests/v5-release-contract.test.mjs","tests/v6-contract-surface.test.mjs","tests/v6-adaptive-flows.test.mjs","tests/v6-orca-boundaries.test.mjs","tests/v6-migration-outcomes.test.mjs"]) ok(packed.includes(wanted),`public package missing ${wanted}`);
