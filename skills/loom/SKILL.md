...
4. A valid parent workspace applies only at its root or within a registered repository; then use workspace-root Loom state. An unregistered sibling remains canonical and may receive optional nearby-workspace guidance. Outside explicit Loom work, an invalid nearby profile warns and disables workspace behavior; explicit Loom invocation fails closed until repaired. Malformed JSON is exceptional: membership cannot be proven, so unregistered descendants may also receive the warning. A valid-profile unregistered sibling remains canonical.
