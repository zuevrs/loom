...
If the host cannot create fresh subagents, execution degrades to solo mode rather than stopping: the coordinator and maker roles merge into one context, and Verify independence is lost — so the loss is marked, never simulated. Self-verify means quoting the diff for every claimed effect and re-reading acceptance line-by-line before reporting. Reports and Ticket records say `solo-verified` and never claim checker independence. Two-strikes REJECT still applies: the second failed self-verify stops for the operator.
...
For prose and reordering work the diff against the authoritative base is the report's core evidence: name its shape — insertions, moves, deletions — and a pure reordering proves every removed line reappears; an insertion that silently eats a neighboring line is exactly the failure this exposes.
