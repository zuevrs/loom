...
2. `session_stop` runs fail-closed artifact validation and canonical Verify evidence checks. Current Loom artifacts that are invalid, or a Ticket marked done without structurally valid independent APPROVE evidence, block every stop attempt until the persisted artifact state is repaired. It does **not** re-prove every historical `done` Ticket against today's live Git checkout; current boundary freshness belongs to Verify/Finish for the selected active Ticket. Stale Workspace bindings block only affected Tickets and are reported explicitly. Chat text and a claimed checker run are not authoritative. Repeated unchanged `session_stop` calls remain blocked and request human repair; they never turn invalid state into a permitted exit.
