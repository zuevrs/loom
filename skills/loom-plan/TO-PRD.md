...
- **PRD** → `.loom/<feature-slug>/PRD.md` via [`PRD-TEMPLATE.md`](PRD-TEMPLATE.md). User stories must be **extensive** — a long numbered list covering every aspect, not just the happy path. Fill Implementation Decisions, Testing Decisions, and the Seams settled during the grill. Every gap the grill left unconfirmed goes into the **Assumptions** section as a one-line reviewable entry — not scattered through the prose. Use the project's `CONTEXT.md` vocabulary throughout; respect ADRs in the area.
