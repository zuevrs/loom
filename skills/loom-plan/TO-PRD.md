...
- **PRD** → `.loom/<feature-slug>/PRD.md` via [`PRD-TEMPLATE.md`](PRD-TEMPLATE.md). User stories must be **extensive** — a long numbered list covering every aspect, not just the happy path. Fill Implementation Decisions, Testing Decisions, and the Seams settled during the grill; research-shaped decisions keep their source links. Every gap the grill left unconfirmed goes into the **Assumptions** section as a one-line reviewable entry — not scattered through the prose. Use the project's `CONTEXT.md` vocabulary throughout; respect ADRs in the area. The PRD remains current until STORY `done`; its concise `## Amendments` pointers explain material changes while the owning sections state the current contract.
...
After confirmation, return to `SKILL.md`; it conditionally loads the shared Orca adapter when configured, then reads [`TO-ISSUES.md`](TO-ISSUES.md) and moves to Phase 3.
