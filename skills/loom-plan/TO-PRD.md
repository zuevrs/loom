...
Use immutable host scratch outside the worktree when supported; otherwise present the full content in chat. Include exact target paths, scope, assumptions, decisions, verification strategy, and all pending domain changes. Omit optional PRD sections when empty; never emit placeholder headings. Compute a draft hash plus base hashes for every existing target.
