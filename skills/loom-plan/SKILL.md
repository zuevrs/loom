...
3. **Issues** — only after PRD confirmation. If the project config resolves to `worktrees: "orca"`, lazy-load [`../loom/ORCA.md`](../loom/ORCA.md), create the story worktrees through its normal bounded confirmation, then return here. Read [`TO-ISSUES.md`](TO-ISSUES.md). Vertical slices, granularity quiz. **Gate 2:** preview the complete issue pack (see Write gates); on confirmation, write issue files via [`ISSUE-TEMPLATE.md`](ISSUE-TEMPLATE.md).
