...
3. **Issues** — only after PRD confirmation. On OMP, lazy-load [`../loom/OMP.md`](../loom/OMP.md) for fresh-worker/context guidance. Plan creates no branches, worktrees, or runtime tasks; Implement previews and creates any Orca lanes just in time. Read [`TO-ISSUES.md`](TO-ISSUES.md). Vertical slices, granularity quiz. **Gate 2:** preview the complete issue pack (see Write gates); on confirmation, write issue files via [`ISSUE-TEMPLATE.md`](ISSUE-TEMPLATE.md).
