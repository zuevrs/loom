...
5. Preview the numbered Ticket breakdown with blockers plus exact target paths, actions, complete bytes, repository owner/base, and write location; the preview's Tickets with blockers are the breakdown approval.
6. Ask one explicit confirmation immediately before writes; it covers granularity and bundle together and permits only that exact inventory. Any post-preview inventory drift requires re-preview.
...
5. Use the bundle mechanics from [`TO-TICKETS.md`](TO-TICKETS.md) for only the affected slices. Preview the complete affected Story/PRD/domain/Ticket delta as one bundle, obtain one bounded confirmation, and write only that bundle. Changed target, action, scope, draft, blocker, repository key, or base requires a fresh complete preview.
