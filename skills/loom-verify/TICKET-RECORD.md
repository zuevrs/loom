...
**Human receipt:** present the result first as one compact line: `Verify: APPROVE|REJECT — Boundary {short digest} · Spec {verdict} · Standards {verdict} · Human {policy}`. The canonical block remains the machine-checkable record; the receipt is a presentation layer and must not replace identities, digests, or cited evidence.
