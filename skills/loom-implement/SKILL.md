...
Read on signal: the Ticket is a bug or a performance regression. Building a feature? Skip to § TDD — the red → green loop.
...
Read on signal: the Ticket involves non-trivial logic; skip for trivial/doc edits — the one-runnable-check rule still applies either way.
