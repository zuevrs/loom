...
1. Exact `/loom tend` from an awaiting-review story is the archive route: lazy-load [`../loom/TEND.md`](../loom/TEND.md), prove every product merge, run `planTendReconciliation` with one writer and semantic confirmation (conflict routes to Grill), then run `planTendArchive` and its digest-bound separate confirmation; owner integration plus archive/readback are required before `done`, then separately run and confirm `planTendCleanup`. No other wording starts archive, monitoring, or cleanup. Otherwise, if the user names a maintenance outcome, inspect only enough evidence to establish it. Bare Tend samples the available signals and recommends/selects the single strongest evidence-backed finding; it does not mandate a full sweep.
