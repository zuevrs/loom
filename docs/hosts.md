...
**On OMP, Orca runs execution; Loom keeps meaning.** Orca owns repositories, worktrees, cards, tasks, dispatches, terminals, and their liveness. Loom keeps the Story, PRD, Tickets, and the current Verify record. The user creates the coordinator worktree/card; work sharing respects blockers and shared resources; worker completion never marks the Story or Ticket done. See [`orca.md`](orca.md).
...
Finish and Publish stay explicit attended boundaries. One exact wave confirmation starts the runnable non-conflicting frontier; newly runnable Tickets wait for the next gate, and wave completion grants no Finish or Publish authority.
