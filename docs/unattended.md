...
3. **Verify still runs.** `loom-verify` before the report. A spec-backed run obtains independent Spec + Standards reviews over one shared evidence packet; Standards-only obtains Standards alone. No parallel workers → independent sequential contexts with the limitation recorded; no independent review → fail closed.
...
Advanced headless checker roles remain available: `LOOM_ROLE=spec-checker omp -p "…"`.
