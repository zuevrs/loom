...
1. In Orca, the user creates the top-level coordinator worktree/card at the validated owner and starts its session. Loom does not create it silently.
2. Setup may offer Orca project wiring, preview the exact project write, and wait for confirmation.
3. Plan records logical repository scope. Merely naming a repository does not create a worktree or task.
4. Implement selects one Ticket, resolves fresh Orca/Git identity, and requests only the execution resources it needs. Within one Story, dependent Tickets run in blocker order; independent repositories may run in parallel. Different Stories may use separate worktrees of the same service. Wait only for a named shared non-Git resource such as a port, database, or container.
5. Worker completion ends only that bounded assignment. It does not mark a Ticket done, close a terminal, commit, push, or publish. The coordinator independently verifies the intended current diff against both Spec and Standards.
